...
except ImportError:  # esecuzione standalone dentro inference/
    from model import Transformer, ModelArgs
...
def qwen_humanize_and_proof(text: str, temperature: float = 0.6, max_new_tokens: int = 32768) -> str:
...
def deepseek_generate_text(prompt: str, temperature: float = 0.9, max_new_tokens: int = 32768) -> str:
...
def llama_generate_text(prompt: str, temperature: float = 0.4, max_new_tokens: int = 8192) -> str:
...
def gemma_generate_json(prompt: str, temperature: float = 0.3, max_new_tokens: int = 8192) -> dict:
...
def analyze_seo_with_gemma(book_text: str) -> dict:
    ...
    data = gemma_generate_json(prompt, temperature=0.25, max_new_tokens=8192)
    return {
...
# Flux cover generation (text-to-image) — FLUX.1-dev via Diffusers
...
def generate_cover_with_flux(title: str, refined_book_text: str, genre: str = None) -> str:
    try:
        ...
        import torch
        prompt = (
            f"Book cover for '{title}'. "
            f"Genre: {genre or ''}. "
            f"Professional layout, evocative atmosphere, key symbols from the story, "
            f"readable typography, editorial design, 8k, detailed."
        )
...
def write_human_style_chapter(chapter_outline: str, style_guide: Dict) -> str:
    ...
    # Nessuna iniezione di refusi: uno strumento editoriale non deve introdurre errori.
...
def api_generate():
    ...
    raw_title = llama_generate_text(llama_title_prompt, temperature=0.5, max_new_tokens=512)
    raw_plot = llama_generate_text(llama_plot_prompt, temperature=0.5, max_new_tokens=4096)
    human_title = qwen_humanize_and_proof(raw_title, temperature=0.6, max_new_tokens=1024).strip()
    human_plot = qwen_humanize_and_proof(raw_plot, temperature=0.6, max_new_tokens=8192).strip()
    ...
    # Outreach editori (via Google CSE) — DISABILITATO di default per evitare invii
    # non richiesti (spam/rischio legale). Abilitare con ENABLE_AUTO_OUTREACH=true
    # solo dopo revisione manuale dei destinatari.
    outreach = {"found": [], "sent": []}
    if os.getenv('ENABLE_AUTO_OUTREACH', '').lower() == 'true':
        ...
        ], attachments=[p for p in [file_path, wattpad_path] if p])
...
def generate_style_guided_text(prompt: str, style_guide: Dict) -> str:
    ...
    """
    style_prompt = (
        "Scrivi seguendo questa guida di stile editoriale:\n"
        f"- Vocabolario: {', '.join(style_guide.get('vocabulary', [])[:5])}\n"
        f"- Struttura delle frasi: {(style_guide.get('sentence_structure') or ['standard'])[0]}\n"
        f"- Temi: {', '.join(style_guide.get('themes', [])[:3])}\n"
        f"- Tecniche narrative: {', '.join(style_guide.get('techniques', [])[:3])}\n\n"
        f"Richiesta: {prompt}"
    )
    # Generazione principale con DeepSeek-V3 (fallback a stringa vuota se non disponibile)
    return deepseek_generate_text(style_prompt, temperature=0.9, max_new_tokens=int(os.getenv('DEEPSEEK_MAX_NEW_TOKENS', '2048')))
